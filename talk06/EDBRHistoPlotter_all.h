...
public:
	...
	EDBRHistoPlotter(std::string nameInDir,
		...
		EDBRColors.resize(33, kWhite);
		EDBRLineColors.resize(33, kWhite);
		std::cout << "Check" << std::endl;
		labels.resize(0);
		labelsSig.resize(0);
//		mcTotalLabels.resize(0);
	...
	void setFillColor(std::vector<int> colorList) {
		unsigned int ind = 0;
		while (ind < 33 && ind < colorList.size()) {
...
//	hs->GetYaxis()->SetNdivisions(404);
	...
	hs->GetYaxis()->SetTitleOffset(1.26);
			...
			for (size_t i = 0; i != histosMCSig.size(); ++i) {
				...
				if (kFactorsSig_.at(i) != 2.0) {
					if (i == 0){
						...
						for(int i=1;i<histosMCSig.size();i++){
							TH1D*h2=(TH1D*)histosMCSig.at(i)->Clone();
							h1->Add(h2,1);
						}
...
//						TString LabelSig = samplesMCSig;
	...
	cout<<"num "<<num<<endl;
	int times=histosMC.size()/7;
	num=(num/times+3)/2-3;
	for (size_t i = 0; i != histosMC.size()/times; ++i) {
		mcTotalLabels.push_back(bkgLegend.at(i));
		TH1D* h1=(TH1D*)histosMC.at(i*times)->Clone();
		for(int k=1;k<times;k++){
			TH1D* h2=(TH1D*)histosMC.at(i*times+k)->Clone();
			h1->Add(h2,1);
		}
		double yerr;
